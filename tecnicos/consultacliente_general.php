...
$sql_clientes = "select nombre,telefono,email,direccion,observaci,idcliente,tipo_operario 
...
echo '<tr><td>NOMBRE</td><td>TIPO</td><td>TELEFONO</td><td>EMAIL</td><td>DIRECCION</td><td>OBSERVACIONES</td></tr>';
$consulta_clientes = mysql_query($sql_clientes,$conexion);
while($clientes = mysql_fetch_array($consulta_clientes))
	{
		 $datos_tipo = consulta_assoc($tipo_operario,'id',$clientes[6],$conexion);		
			echo '<tr>';	
			echo '<td><a href = "muestre_datos_cliente.php?idcliente='.$clientes[5].' " >'.$clientes[0].'</a></td>';
			echo '<td>'.$datos_tipo['descripcion'].'</td>';
