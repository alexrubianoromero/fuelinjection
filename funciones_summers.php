...
//   $con = mysql_query($sql_cliente,$conexion);
// }
function suma_abonos($idcuenta,$conexion,$tabla23){
   $sql = "select sum(lasumade) as valor  from $tabla23 where id_cxp = '".$idcuenta."'";
  //  echo $sql;
   $consulta = mysql_query($sql,$conexion);
   $arr = mysql_fetch_assoc($consulta);
   return $arr['valor']; 
}
/////traer el nombre de alguien de tabla cliente 0
 function traer_nombre($tabla,$id_cliente,$conexion){
  ...
  //traer datos  orden 
  function traer_orden($tabla,$campo,$id,$conexion){
    $sql_cliente= "select * from $tabla where $campo = '".$id."'  ";
    $con_cliente = mysql_query($sql_cliente,$conexion);
    $arr_cliente = mysql_fetch_assoc($con_cliente);
    $identi = $arr_cliente['orden'];
    return  $identi;
}
