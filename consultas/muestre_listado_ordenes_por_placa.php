...
$sql_listado_ordenes = "select id,fecha,orden,estado,kilometraje,kilometraje_cambio,anulado from $tabla14 where placa = '".$_POST['placa123']."' and id_empresa = '".$_SESSION['id_empresa']."' ";

$traer_vencimientos  = "select vencisoat,revision    from  $tabla4   where placa = '".$_POST['placa123']."' ";
...
echo '<tr><td><h3>VENCIMIENTO SOAT</h3></td><td><h3>VENCIMIENTO TECNOMECANICA</h3></td></tr>';
echo '<tr><td><h3>'.$vencimientos['vencisoat'].'</h3></td><td><h3>'.$vencimientos['revision'].'</h3></td></tr>';
echo '</table>';
