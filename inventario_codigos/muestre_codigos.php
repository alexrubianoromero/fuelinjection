...
if($filas == 0 )
		{
	...
	else
	 	{
			...
			//draw_table($codigos);
	 		echo '<table border = "1">';
	 		 echo '<tr>';
	 		 echo '<td><h3>CODIGO</td><td><h3>DESCRIPCION</h3></td><td><h3>VALOR COMPRA</h3></td><td><h3>VALOR VENTA(Sin Iva)</h3></td><td><h3>EXISTENCIAS </h3></td><td><h3>COMPRAS</h3></td><td><h3>MODIFICAR</h3></td>';
	 		 echo '</tr>';
			  while($codigos = mysql_fetch_array ($consulta_codigos))
			  		{
			  			echo '<tr>';
			  				echo '<td><h3>'.$codigos[0].'</h3></td><td><h3>'.$codigos[1].'</h3></td><td><h3>'.$codigos[2].'</h3></td><td><h3>'.$codigos[5].'</h3></td><td><h3>'.$codigos[3].'</h3></td>';
